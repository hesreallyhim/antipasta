...
| `duplication_ratio` | DRYness | File | `0..1` | Threshold applies only when `duplication.max_ratio` is set | Python | `duplication` config block | pydry plus custom | Per-file duplicated-line ratio from pydry clone groups. |
