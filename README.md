...
When running `antipasta generate-config`, you'll see prompts like:
...
|---------|-------------|------------------|
| `generate-config` | Generate a configuration file interactively or with defaults | Creates `.antipasta.yaml` in current directory |
| `metrics` | Analyze code metrics for specified files | Uses `.antipasta.yaml` config, shows helpful message if missing |
| `validate-config` | Validate a configuration file | Validates `.antipasta.yaml` if no file specified |
...
1. Show a helpful message: "Configuration file '.antipasta.yaml' not found. Run 'antipasta generate-config' to create a configuration file."
...
# Generated by: antipasta generate-config
...
antipasta validate-config && antipasta metrics --directory src/
```
