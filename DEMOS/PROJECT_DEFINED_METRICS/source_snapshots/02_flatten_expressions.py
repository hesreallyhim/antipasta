"""Snapshot 02: expression flatness improved; other smells remain."""
...
def process_orders(
    ...
    for row in rows:
        try:
            ...
            is_duplicate = feature_flags.get("dedupe") and cache_key in order_cache
            if is_duplicate:
                continue
            ...
            missing = is_ready(order)
            if missing:
                failed.append({"id": row["id"], "missing": missing})
                continue
            ...
            email = proc2(order)
            receipt = render_invoice(row["id"], email, amount, sku)
            total += amount
